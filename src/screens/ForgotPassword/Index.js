    ...
    <>
      <View className="italic" style={styles.container}>
        ...
        <View>
          ...
          {reqSent && <Text className="mx-6 my-8 text-red-600">An OTP will be sent to this address if it exists on our database</Text>}

          {
            emailSent && <View className="mx-6 flex-row">
              <Text >Received an OTP? </Text>
              <Text className="mx-1 text-primary" onPress={() => {
                if (!reqSent) {
                  Toast.show({
                    type: "error",
